...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'confirmer') {
        ...
        $message = "<div class='alert-success'><i class='fas fa-check'></i> Réservation confirmée !</div>";
    }
    if ($_POST['action'] === 'annuler') {
        $pdo->prepare("UPDATE reservation SET statut = 'ANNULEE' WHERE id_reservation = ?")->execute([$id]);
        $message = "<div class='alert-danger'><i class='fas fa-xmark'></i> Réservation annulée.</div>";
    }
    if ($_POST['action'] === 'honorer') {
        $pdo->prepare("UPDATE reservation SET statut = 'HONOREE' WHERE id_reservation = ?")->execute([$id]);
        $message = "<div class='alert-success'><i class='fas fa-graduation-cap'></i> Réservation honorée !</div>";
    }
}
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="reservations.php"    class="sidebar-link active"><span class="icon"><i class="fas fa-calendar-alt"></i></span> Réservations</a>
...
<div class="main-wrap">
    ...
    <div class="content">
        ...
        <div class="stats-grid">
            <div class="stat-card">
                <div class="stat-icon" style="background:#f0f0f0;"><i class="fas fa-clipboard-list"></i></div>
                <div><div class="stat-label">Total</div><div class="stat-value" style="color:#1a1a2e;"><?= array_sum($all) ?></div></div>
            </div>
            <div class="stat-card">
                <div class="stat-icon" style="background:#fff3e0;"><i class="fas fa-hourglass-half"></i></div>
                <div><div class="stat-label">En attente</div><div class="stat-value" style="color:#e65100;"><?= $all['EN_ATTENTE'] ?? 0 ?></div></div>
            </div>
            <div class="stat-card">
                <div class="stat-icon" style="background:#e8f4fd;"><i class="fas fa-check"></i></div>
                <div><div class="stat-label">Confirmées</div><div class="stat-value" style="color:#0081c9;"><?= $all['CONFIRMEE'] ?? 0 ?></div></div>
            </div>
            <div class="stat-card">
                <div class="stat-icon" style="background:#e8f8f0;"><i class="fas fa-graduation-cap"></i></div>
        ...
        <div class="table-card">
            <div class="table-header">
                <h5><i class="fas fa-clipboard-list"></i> Réservations</h5>
                <div class="filtres">
                    <a href="?filtre=TOUS"      class="filtre-btn <?= $filtre==='TOUS'      ?'active':'' ?>">Tous</a>
                    <a href="?filtre=EN_ATTENTE" class="filtre-btn <?= $filtre==='EN_ATTENTE'?'active':'' ?>"><i class="fas fa-hourglass-half"></i> En attente</a>
                    <a href="?filtre=CONFIRMEE"  class="filtre-btn <?= $filtre==='CONFIRMEE' ?'active':'' ?>"><i class="fas fa-check"></i> Confirmées</a>
                    <a href="?filtre=HONOREE"    class="filtre-btn <?= $filtre==='HONOREE'   ?'active':'' ?>"><i class="fas fa-graduation-cap"></i> Honorées</a>
                    <a href="?filtre=ANNULEE"    class="filtre-btn <?= $filtre==='ANNULEE'   ?'active':'' ?>"><i class="fas fa-xmark"></i> Annulées</a>
                </div>
            </div>
            <table>
                <thead>
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($reservations as $r):
                            $badges = ['EN_ATTENTE'=>['badge-attente','<i class="fas fa-hourglass-half"></i> En attente'],'CONFIRMEE'=>['badge-confirmee','<i class="fas fa-check"></i> Confirmée'],'ANNULEE'=>['badge-annulee','<i class="fas fa-xmark"></i> Annulée'],'HONOREE'=>['badge-honoree','<i class="fas fa-graduation-cap"></i> Honorée']];
                            $b = $badges[$r['statut']] ?? ['',''];
                        ?>
                        <tr>
                            ...
                            <td>
                                <?php if ($r['statut'] === 'EN_ATTENTE'): ?>
                                    <form method="POST" style="display:inline;">
                                        ...
                                        <button type="submit" class="btn-action btn-confirmer"><i class="fas fa-check"></i> Confirmer</button>
                                    </form>
                                    <form method="POST" style="display:inline;">
                                        <input type="hidden" name="action" value="annuler">
                                        <input type="hidden" name="id_reservation" value="<?= $r['id_reservation'] ?>">
                                        <button type="submit" class="btn-action btn-annuler"><i class="fas fa-xmark"></i> Annuler</button>
                                    </form>
                                <?php elseif ($r['statut'] === 'CONFIRMEE'): ?>
                                    <form method="POST" style="display:inline;">
                                        <input type="hidden" name="action" value="honorer">
                                        <input type="hidden" name="id_reservation" value="<?= $r['id_reservation'] ?>">
                                        <button type="submit" class="btn-action btn-honorer"><i class="fas fa-graduation-cap"></i> Honorer</button>
                                    </form>
                                <?php else: ?>
