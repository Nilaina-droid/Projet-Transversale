...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'ajouter') {
        ...
        // 1. Récupération et vérification de l'id_service
        $id_service       = !empty($_POST['id_service']) ? $_POST['id_service'] : null;

        if (!empty($_POST['plats'])) {
            if (is_null($id_service)) {
                // Si l'utilisateur n'a pas sélectionné de service, on bloque avant le SQL
                $message = "<div class='alert-danger'>⚠️ Erreur : Vous devez sélectionner un service valide pour y associer des plats.</div>";
            } else {
                // Tout est bon, on procède à l'insertion du menu
                ...
                $message = "<div class='alert-success'>✅ Menu publié avec succès !</div>";
            }
        } else {
            // Permet de publier un menu vide (Brouillon par exemple) sans générer d'erreurs de quota
            $stmt = $pdo->prepare("INSERT INTO menu (semaine_du, semaine_au, date_publication, statut, id_gestionnaire) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$semaine_du, $semaine_au, $date_publication, $statut, $id_gestionnaire]);
            $message = "<div class='alert-success'>✅ Menu (sans plats) publié avec succès !</div>";
        }
    }
    ...
    if ($_POST['action'] === 'supprimer') {
        ...
        $message = "<div class='alert-danger'>🗑️ Menu supprimé.</div>";
    }
    ...
    if ($_POST['action'] === 'changer_statut') {
        ...
        $message = "<div class='alert-success'>✅ Statut mis à jour !</div>";
    }
}
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .btn-top { display: inline-flex; align-items: center; gap: 6px; background: #0081c9; color: #fff; padding: 9px 18px; border-radius: 8px; font-size: 13px; font-weight: 700; text-decoration: none; transition: background 0.2s; }
        ...
        select {
            ...
            height: 38px; transition: border-color 0.2s, box-shadow 0.2s;
        }
        ...
        .plat-checkbox label { cursor: pointer; font-size: 13px; font-weight: 500; color: #333; text-transform: none; letter-spacing: 0; margin: 0; }

        .no-plat-warning { background: #fff8e1; border-left: 4px solid #ffa000; color: #e65100; padding: 12px 16px; border-radius: 8px; font-size: 13px; font-weight: 600; }
        ...
        /* ── BTN SUPPRIMER ── */
        .btn-suppr { display: inline-block; background: #fdecea; color: #c0392b; padding: 5px 10px; border-radius: 6px; font-size: 11px; font-weight: 600; border: none; cursor: pointer; transition: opacity 0.2s; margin-left: 6px; }
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="gestion_menus.php"   class="sidebar-link active"><span class="icon">🍽️</span> Gestion des menus</a>
...
<div class="main-wrap">
    <div class="topbar">
        ...
        <a href="gestion_plats.php" class="btn-top">🥘 Gérer les plats</a>
    </div>
            ...
            <div class="card-form-body">
                <form method="POST">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            <label>Semaine au</label>
                            <input type="date" name="semaine_au" required>
                        </div>
                        <div class="form-group">
                            <label>Places max / plat</label>
                        ...
                        <div class="form-group full">
                            <label>🍽️ Plats du menu <span style="font-weight:400;color:#aaa;text-transform:none;">(cochez les plats disponibles)</span></label>
                            <?php if (empty($plats)): ?>
                                <div class="no-plat-warning">
                                    ⚠️ Aucun plat disponible. <a href="gestion_plats.php">Ajoutez des plats d'abord !</a>
                                </div>
                            <?php else: ?>
                                <div class="plats-grid">
                                    <?php
                                    $emoji_types = ['Viande'=>'🥩','Poisson'=>'🐟','Légumes'=>'🥦','Dessert'=>'🍮','Boisson'=>'🥤','Entrée'=>'🥗','Féculent'=>'🍚'];
                                    foreach ($plats as $p):
                                        $icon = $emoji_types[$p['type_plat']] ?? '🍽️';
                                    ?>
                                    <div class="plat-checkbox">
                                        <input type="checkbox" name="plats[]" value="<?= $p['id_plat'] ?>" id="plat_<?= $p['id_plat'] ?>">
                                        <label for="plat_<?= $p['id_plat'] ?>"><?= $icon ?> <?= htmlspecialchars($p['nom_plat']) ?></label>
                                    </div>
                                    <?php endforeach; ?>
                                </div>
                            <?php endif; ?>
                    ...
                    <button type="submit" class="btn-submit">📤 Publier le menu</button>
                </form>
            </div>
        </div>
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php
                                $badges = ['PUBLIE'=>['badge-publie','✅ Publié'],'BROUILLON'=>['badge-brouillon','📝 Brouillon'],'ARCHIVE'=>['badge-archive','📦 Archivé']];
                                $b = $badges[$m['statut']] ?? ['',''];
                                ?>
